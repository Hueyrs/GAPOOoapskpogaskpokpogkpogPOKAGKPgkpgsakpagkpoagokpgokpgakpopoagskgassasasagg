...
public class MeleeCombatAction extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		if(attacker.getSkills().getLevel(Skills.HITPOINTS) < 1 || victim.getSkills().getLevel(Skills.HITPOINTS) < 1) {
			return;
		}
		if(!attacker.canEmote()) {
			return;
		}
		...
		World.getWorld().submit(new Tickable(1) {
			...
			public void execute() {
				if(!victim.canEmote()) {
					if(victim.delayEmoteHits.isEmpty()) {
						World.getWorld().submit(new Tickable((victim.canEmoteWait - 4)) {
							public void execute() {
								final ArrayList<Integer> doneGhostHits = new ArrayList<Integer>();
								for(int ghostHit : victim.delayEmoteHits) {
									if(ghostHit >= 0 && !doneGhostHits.contains(ghostHit)) {
										victim.inflictDamage(new Hit(ghostHit), attacker);
										doneGhostHits.add(ghostHit);
									}
								}
								doneGhostHits.clear();
								victim.delayEmoteHits.clear();
								this.stop();
							}
						});
					}
					victim.delayEmoteHits.add(hit);
					System.out.println(victim.delayEmoteHits);
					this.stop();
					return;
				}
