...
public class ChaosElemental extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		switch(style) {
		case DISARM:
			attacker.playAnimation(Animation.create(3144));
	...
	private void applyEffect(final Mob victim, CombatStyle style) {
		...
		case DISARM:
			...
			for(@SuppressWarnings("unused") int slot : slots) {
				if(victim.getEquipment() != null) {
					//victim.getEquipment().remove(victim.getEquipment().)
					//TODO: Finish tommarow
				}
			}
			break;
		}
	...
	private Location generateLocation() {
		Location loc = Location.create(3219 + Misc.random(3), 3917 + Misc.random(3));
		if(Misc.random(1) > 0) {
