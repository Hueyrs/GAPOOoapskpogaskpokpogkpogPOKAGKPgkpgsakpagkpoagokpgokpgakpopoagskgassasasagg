...
public class BarbAssualt {
	...
	public void update() {
		for(Player pz : players) {
			if(pz != null) {
				...
				pz.getActionSender().sendString(194, 1, "Barbarian Assualt");
				...
				pz.getActionSender().sendString(194, 6, "Assualt Points: ");
				pz.getActionSender().sendString(194, 11, Integer.toString(pz.assualtPoints));
				int blank[] = { 7, 12, 8, 13 };
				for(int i : blank) {
	...
	public void endGame(boolean won) {
		...
		ArrayList<Player> completed = new ArrayList<Player>();
		for(Player pz : players) {
			if(pz != null && !completed.contains(pz)) {
				pz.setTeleportTarget(pz.DEFAULT_LOCATION);
				pz.getActionSender().removeAllInterfaces();
				if(won) {
					pz.getActionSender().sendMessage("Congratulations! You've defeated Barbarian Assualt!");
